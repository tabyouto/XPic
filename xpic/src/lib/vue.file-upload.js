...
// define
//btn btn-default btn-xs btn-upload
var FileUploadComponent = Vue.extend({
    template: '<div class="{{ class }}">' +
    '<div class="btn btn-default btn-xs btn-upload">选择文件' +
    '<label for="{{ name }}">' +
    '<input type="file" name="{{ name }}" id="{{ id || name }}"  v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}">' +
    '</div>'+
    '<slot></slot>' +
    '</label>' +
    '</div>'+
    '<button type="button" :disabled="this.myFiles.length==0" v-on:click="fileUpload">上传</button>',
    props: {
        class: String,
        name: {
            type: String,
    ...
    methods: {
        ...
        fileInputChange: function () {
            ...
            this.$dispatch('onFileChange', this.myFiles); //输出选中的文件名称
        },
